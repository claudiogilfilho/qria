...
describe("procedimentos de marca", () => {
  ...
  it("monta o workspace com marca, sessão, direções e escolha persistida", async () => {
    ...
    const directions = [{ id: 93, sessionId: 72, title: "Âmbar Editorial", round: 1, optionKey: "B", status: "selected" }];
    ...
    await expect(caller.brand.getWorkspace({ brandId: 41 })).resolves.toEqual({ brand, session, directions, selectedDirection: directions[0] });
  });
