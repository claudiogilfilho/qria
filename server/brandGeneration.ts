...
export function createFallbackDirections(input: GenerationInput): BrandDirection[] {
  const round = Math.floor(input.priorDirectionTitles.length / 4) + 1;
...
export function createFallbackLogo(direction: BrandDirection, index = 0) {
  ...
  const rotation = (index * 23) % 90;
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 240 240" role="img" aria-label="Símbolo conceitual"><rect width="240" height="240" rx="36" fill="${background}"/><g transform="rotate(${rotation} 120 120)" fill="none" stroke="${highlight}" stroke-width="9" stroke-linecap="round" stroke-linejoin="round"><path d="M120 42C74 42 57 79 57 116c0 46 29 77 63 82"/><path d="M120 67c-29 0-38 25-38 49 0 28 16 49 38 57"/><path d="M120 42c46 0 63 37 63 74 0 46-29 77-63 82"/><path d="M120 67c29 0 38 25 38 49 0 28-16 49-38 57"/><path d="M120 91v107"/></g><circle cx="120" cy="120" r="17" fill="${signal}"/></svg>`;
